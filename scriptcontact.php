...
//on vérifie que le formulaire a été soumis
    ...
    //on crée des fonctions pour vérifier la validité de chaque champ
        ...
        function valider_demande($demande){
            $regex = "/^[A-Za-z]+$/";
            if (preg_match($regex, $demande)){
                return true;
            }
            else{
        ...
        if (valider_nom($nom) == true && valider_prenom($prenom) == true && valider_email($email) == true && valider_tel($tel) == true && valider_demande($demande) == true){
            //on crée une session
            // $_SESSION["nom"] = $nom;
